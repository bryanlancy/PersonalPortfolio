...
export default function ProjectContainer({ image, icon, title, sub, desc, left }) {
	...
	return (
		<div className="project" onMouseEnter={hoverStyle} onMouseLeave={hoverStyle}>
			<img className="project__image" src={require(`../images/${image}`).default} alt={`${title}-project`} style={{ [left ? 'left' : 'right']: '-24px' }} />
